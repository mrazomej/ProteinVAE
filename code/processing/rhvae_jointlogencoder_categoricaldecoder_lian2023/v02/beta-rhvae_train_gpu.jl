...
# Define logp_prefactor
logp_prefactor = Float32.([10, 1, 1])

# Generate loss function kwargs
loss_kwargs = Dict(
    :K => K,
    :ϵ => ϵ,
    :βₒ => βₒ,
...
val_data = CUDA.CuArray(train_data)
...
for epoch in epoch_init:n_epoch
    ...
    # Save checkpoint
    JLD2.jldsave(
        "$(out_dir)/beata-rhvae_GPU_epoch$(lpad(epoch, 5, "0")).jld2",
        model_state=Flux.state(rhvae)|> Flux.gpu,
        loglike_train=loglike_train,
        loglike_val=loglike_val,
        loss_train=loss_train,
        loss_val=loss_val
